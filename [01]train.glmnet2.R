...
fn.opt <- function(pars) {     
  -auc(pred.train$action[ix.sample], fn.opt.pred(pars, pred.train[ix.sample,cols])) 
} 
...
for (iter in 2:300) {
  ...
  ix.sample <- sample((1:nrow(pred.train)),round(0.9*nrow(pred.train)))
  cols <- c("prev", "cur") 
